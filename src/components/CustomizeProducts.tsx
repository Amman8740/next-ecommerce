        ...
        <div className="flex flex-col gap-4" key={option.name}>
          ...
          {option.choices?.map((choice) => {
            ...
            return option.name === "Color" ? (
              <li key={`${option.name}-${choice.description}`} className="w-8 h-8 rounded-full ring-1 ring-gray-300 relative" style={{backgroundColor:choice.value, cursor: disabled ? "not-allowed" : "pointer"}}
              ...
              <li key={`${option.name}-${choice.description}`} className="ring-1 ring-lama text-lama rounded-md py-1 px-4 text-sm" style={{cursor: disabled ? "not-allowed" : "pointer",
